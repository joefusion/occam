...
bool SBMManager::initFromCommandLine(int argc, char **argv) {
    ...
    if (varList) {
        ...
        Relation *top = getRelation(var_indices, var_count, false, state_indices);
...
void SBMManager::verifyStateBasedNaming() {
...
void SBMManager::makeReferenceModels(Relation *top) {
    ...
    if (varList->isDirected()) {
        ...
        for (i = 0; i < varCount; i++) {
            ...
            /*
             ...
             long cellCount = 0;
             for(;;) {
             Key::buildFullKey(key, keysize, varList, varvalues);
    ...
    if (!modelCache->addModel(bottomRef)) {
        Model *cached_model = modelCache->findModel(bottomRef->getPrintName());
        delete bottomRef;
...
Model *SBMManager::setRefModel(const char *name) {
...
double SBMManager::computeExplainedInformation(Model *model) {
...
double SBMManager::computeUnexplainedInformation(Model *model) {
    double topH = topRef->getAttribute(ATTRIBUTE_H);
    double botH = bottomRef->getAttribute(ATTRIBUTE_H);
    double modelT = computeTransmission(model, IPF);
...
double SBMManager::computeDDF(Model *model) {
...
void SBMManager::setSearch(const char *name) {
    if (search) {
        delete search;
        search = NULL;
    }
...
void SBMManager::computeDFStatistics(Model *model) {
    computeDfSb(model);
    computeDDF(model);
}

void SBMManager::computeInformationStatistics(Model *model) {
...
void SBMManager::computeL2Statistics(Model *model) {
...
void SBMManager::computePearsonStatistics(Model *model) {
...
void SBMManager::computeDependentStatistics(Model *model) {
...
void SBMManager::computeBPStatistics(Model *model) {
    ...
    class BPIntersectProcessor: public ocIntersectProcessor {
        public:
            BPIntersectProcessor(Table *inData, int rels, long fullDim) {
                inputData = inData;
...
void SBMManager::computePercentCorrect(Model *model) {
    double total;
    long long count, i;
    ...
    Relation *predRelNoDV = getRelation(varindices, varcount, false, stateindices);
    getPredictingVars(model, varindices, varcount, true);
    for (int i = 0; i < varcount; i++)
        stateindices[i] = DONT_CARE;
    Relation *predRelWithDV = getRelation(varindices, varcount, false, stateindices);
...
bool SBMManager::applyFilter(Model *model) {
...
void SBMManager::setSortAttr(const char *name) {
    if (sortAttr)
        delete sortAttr;
...
static void printRefTable(Model *model, FILE *fd, const char *ref, const char **strings,
        int rows) {
    ...
    if (Report::isHTMLMode()) {       // does this check a particular Report object, or the class?  seems odd.
        line_sep = "<hr>\n";
...
void SBMManager::printFitReport(Model *model, FILE *fd) {
    //-- Print general report for a single model, similar to Fit in Occam2
...
void SBMManager::printBasicStatistics() {
